...
CALL db.index.fulltext.queryNodes("txt_index_speciality", "Obstetrics") YIELD node, score RETURN node
...
MATCH (p:Patient{patient_id:"PT005"})-[r:BE_TREATED]->(doc:Doctor{doctor_id:"DR.011"})
WHERE r.end_date IS NULL
SET r.diagnosis = "Benh sot"
...
CREATE (p)-[:BE_TREATED {start_date: date("2025-04-02")}]->(d);
